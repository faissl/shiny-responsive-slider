<?php
/*
	Plugin Name: Shiny Responsive Slider
	Plugin URI: http://www.studiopress.com
	Description: A HigHlY HaCkEd version of the Genesis responsive featured 
	             slider for the Genesis Framework.
	Author: StudioPress
...
define( 'SHINY_RESPONSIVE_SLIDER_SETTINGS_FIELD', 'shiny_responsive_slider_settings' );
define( 'SHINY_RESPONSIVE_SLIDER_VERSION', '0.0' );

add_action( 'after_setup_theme', 'ShinyResponsiveSliderInit', 15 );
/**
 * Loads required files and adds image via Genesis Init Hook
 */
function ShinyResponsiveSliderInit() {
	...
	// translation support
	load_plugin_textdomain( 'shiny-responsive-slider', false, '/shiny-responsive-slider/languages/' );
	
	/** hook all frontend slider functions here to ensure Genesis is active **/
	add_action( 'wp_enqueue_scripts', 'shiny_responsive_slider_scripts' );
	add_action( 'wp_print_styles', 'shiny_responsive_slider_styles' );
	add_action( 'wp_head', 'shiny_responsive_slider_head', 1 );
	add_action( 'wp_footer', 'shiny_responsive_slider_flexslider_params' );
	add_action( 'widgets_init', 'shiny_responsive_sliderRegister' );
...
add_action( 'genesis_settings_sanitizer_init', 'shiny_responsive_slider_sanitization' );
/**
 * Add settings to Genesis sanitization
 *
 */
function shiny_responsive_slider_sanitization() {
	genesis_add_option_filter( 'one_zero', SHINY_RESPONSIVE_SLIDER_SETTINGS_FIELD,
		array(
			'slideshow_arrows',
	...
	genesis_add_option_filter( 'no_html', SHINY_RESPONSIVE_SLIDER_SETTINGS_FIELD,
		array(
			'post_type',
			'posts_term',
			'exclude_terms',
 ...
 */
function shiny_responsive_slider_scripts() {

	/** easySlider JavaScript code */
	wp_enqueue_script( 'flexslider', plugins_url('js/jquery.flexslider.js', __FILE__), array( 'jquery' ), SHINY_RESPONSIVE_SLIDER_VERSION, TRUE );
 ...
 * Load the CSS files
 */
function shiny_responsive_slider_styles() {

	/** standard slideshow styles */
	wp_register_style( 'slider_styles', plugins_url('style.css', __FILE__), array(), SHINY_RESPONSIVE_SLIDER_VERSION );
 ...
 */
function shiny_responsive_slider_head() {
 ...
 */
function shiny_responsive_slider_flexslider_params() {
	...
	$output = 'jQuery(document).ready(function($) {
				$(".flexslider").flexslider({
					controlsContainer: "#shiny-responsive-slider",
 ...
 */
function shiny_responsive_sliderRegister() {
	register_widget( 'shiny_responsive_sliderWidget' );
}

/** Creates read more link after excerpt */
function shiny_responsive_slider_excerpt_more( $more ) {
	global $post;
	...
	return '&hellip; <a href="'. get_permalink( $post->ID ) . '">' . __( $read_more, 'shiny-responsive-slider' ) . '</a>';
}

/**
 * Slideshow Widget Class
 */
class shiny_responsive_sliderWidget extends WP_Widget {

		function shiny_responsive_sliderWidget() {
			$widget_ops = array( 'classname' => 'shiny_responsive_slider', 'description' => __( 'Displays a slideshow inside a widget area', 'shiny-responsive-slider' ) );
			$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'shinyresponsiveslider-widget' );
			$this->WP_Widget( 'shinyresponsiveslider-widget', __( 'Shiny Genesis - Responsive Slider', 'shiny-responsive-slider' ), $widget_ops, $control_ops );
		}
		...
		function widget( $args, $instance ) {
			...
			$query_args = apply_filters( 'shiny_responsive_slider_query_args', $query_args );
			add_filter( 'excerpt_more', 'shiny_responsive_slider_excerpt_more' );
		
?>

		<div id="shiny-responsive-slider">			
			...
			<?php	
			/*** Hook right after the slider area. 	
			     Provides a means to add a static area after the sliders, but within the widget bounds. ***/
			if ( has_action( 'shiny_after_slider' ) ) do_action( 'shiny_after_slider' );
			?>


		</div><!-- end #shiny-responsive-slider -->

		<?php
		echo $after_widget;
		wp_reset_query();
		remove_filter( 'excerpt_more', 'shiny_responsive_slider_excerpt_more' );
...
?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'shiny-responsive-slider' ); ?> <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></label></p>
<?php
			echo '<p>';
			printf( __( 'To configure slider options, please go to the <a href="%s">Slider Settings</a> page.', 'shiny-responsive-slider' ), menu_page_url( 'shiny_responsive_slider', 0 ) );
			echo '</p>';
		}
 ...
 * @return string
 */
function shiny_responsive_slider_exclude_taxonomies( $taxonomy ) {

	$filters = array( '', 'nav_menu' );
	$filters = apply_filters( 'shiny_responsive_slider_exclude_taxonomies', $filters );
 ...
 * @return string
 */
function shiny_responsive_slider_exclude_post_types( $type ) {

	$filters = array( '', 'attachment' );
	$filters = apply_filters( 'shiny_responsive_slider_exclude_post_types', $filters );
...
function genesis_get_responsive_slider_option( $key ) {
	return genesis_get_option( $key, SHINY_RESPONSIVE_SLIDER_SETTINGS_FIELD );
}
 ...
 * @param string $key key value for option
 */
function shiny_responsive_slider_option( $key ) {
